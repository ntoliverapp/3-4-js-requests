...
// CODE HERE
function colorChangeBack(event) {
    event.preventDefault();
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/response')
    .then(response => {
      console.log(response.data)
...
After doing this you still won't see anything on the page, that is because the repeat-text element is set to display: none. In order for this to work correctly you will need to set the display to block.
*/
...
const getQueryRequest = () => {
    axios.get('http://localhost:3000/query-test/?name=niq')
    .then(response => {
        console.log(response.data)
        const helloText = document.getElementById('hello-text') 
      helloText.textContent = response.data
      helloText.style.display = "block"
    })
}
document.getElementById('query-button').addEventListener("click",getQueryRequest)
